      ...
      >
        {isSignedIn && user ? (
          <Card appearance="outlined" style={{ gap: theme.spacing.md }}>
            <View style={[styles.row, { gap: theme.spacing.md }]}>
              ...
              >
                <Text variant="h2" color={theme.colors.primary}>
                  {(user.businessName ?? user.fullName ?? user.email).slice(0, 1).toUpperCase()}
                </Text>
              </View>

              <View style={{ flex: 1, gap: 2 }}>
                <Text variant="h3" numberOfLines={1}>
                  {user.businessName ?? user.fullName ?? 'حساب جملة'}
                </Text>
            ...
            {(user.iceNumber || user.phone) && (
              <>
                <Divider />
                <View style={{ gap: theme.spacing.xs }}>
                  {user.iceNumber && <InfoRow label="رقم ICE" value={user.iceNumber} />}
                  {user.phone && <InfoRow label="الهاتف" value={user.phone} />}
                </View>
              </>
            )}

            <Badge label="حساب جملة" tone="primary" icon="business-outline" />
          </Card>
        ) : (
          <Card appearance="outlined" style={{ gap: theme.spacing.md }}>
            <Text variant="h3">لم تسجّل الدخول بعد</Text>
            <Text variant="body" muted>
              سجّل الدخول أو أنشئ حساب جملة لإرسال الطلبات ومتابعتها.
            </Text>
        ...
        <View style={{ gap: theme.spacing.sm }}>
          ...
          <Card appearance="outlined" padding="none" style={{ overflow: 'hidden' }}>
            ...
            <MenuRow
              ...
              value={
                paymentMethods.data
                  ? `${paymentMethods.data.active.toUpperCase()} · بطاقة فقط`
                  : 'جارٍ التحميل…'
              }
            />
            <Divider />
