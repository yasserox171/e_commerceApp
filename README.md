...
كيجيب الفرع ← كيثبّت ← كيبني ← كيطبّق الترحيلات ← كيعاود التشغيل ← كيفحص
`/health`. وأي خطوة كتفشل كتوقفو قبل إعادة التشغيل، فالنسخة القديمة كتبقى خدامة.

ثلاث قرارات فالتصميم خاصك تعرفهم:

- **الأسرار ف `/etc/qri3a/api.env`**، برّا المشروع — `git pull` عمرو يمسحهم،
  وملف الخدمة ديال systemd (اللي كيقراه الجميع) ما فيه حتى credential
- **دور قاعدة البيانات ما عندوش صلاحية الكتابة ف `public`** — أسوأ bug ممكن
  ما يقدرش يخسّر الكتالوج. كلشي لي كتكتب الواجهة كيعيش ف `commerce`
- **الـ API مربوطة على `127.0.0.1`** — حتى لو تحلّ الجدار الناري غلط، ما كاين
  حتى socket عمومي على 4000

**الدفع مؤجّل بقصد:** الواجهة كتقلع وكتخدم بلا مفاتيح CMI —
`POST /payments/checkout` كيرد `503` برسالة واضحة حتى تعمّرهم، و `/health`
كيبيّن `payments.configured: false`. هادي حالة عادية حتى يتوقّع عقد CMI.
